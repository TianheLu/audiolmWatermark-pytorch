...
class MultiScaleMessageExtractor(nn.Module):
    # the result is so similar, consider other dims
    def __init__(
        self,
        msg_len = 4,
        channels = 16,
        ...
        self.final_conv = nn.Sequential(
            nn.Conv1d(curr_channels, curr_channels, 5, padding = 2),
            leaky_relu(),
            nn.Conv1d(curr_channels, msg_len, 3, padding = 1),
        )
...
class ModReLU(nn.Module):
    ...
    # where 1.j represents the complex unit
...
def ComplexSTFTResidualUnit(chan_in, chan_out, strides):
    ...
    return nn.Sequential(
        Residual(Sequential(ComplexConv2d(chan_in, chan_in, 3, padding=1),
                            ModReLU(),
...
class ComplexSTFTDiscriminator(nn.Module):
    def __init__(self, *, channels=32, strides=((1,2), (2,2), (1,2), (2,2), (1,2), (2,2)),
                 chan_mults=(1, 2, 4, 4, 8, 8), input_channels=1, n_fft=1024, hop_length=256,
                 win_length=1024, stft_normalized=False, logits_abs=True):
        super().__init__()
        ...
        self.final_conv = ComplexConv2d(layer_channels[-1], 1, (16, 1)) # todo: remove hard coded 16
    ...
    def forward(self, x, return_intermediates=False):
        ...
        x = torch.stft(x, self.n_fft, hop_length=self.hop_length,
                       win_length=self.win_length, normalized=self.stft_normalized,
                       return_complex=True)
        x = rearrange(x, 'b ... -> b 1 ...')
        intermediates = []
        x = self.init_conv(x)
...
class ComplexSTFTMessageExtractor(nn.Module):
    def __init__(
        self,
        *,
        msg_len = 4,
        ...
        self.final_conv = ComplexConv2d(layer_channels[-1], msg_len, (16, 1)) # todo: remove hardcoded 16
        ...
        self.msg_out = nn.Linear(100, msg_len)
    ...
    def forward(self, x, return_intermediates = False):
        ...
        decoded_msg_out = complex_logits[:, :, -1, -1]

        if not return_intermediates:
            return decoded_msg_out
        return decoded_msg_out, intermediates
...
class CausalConv1d(nn.Module):
    def __init__(self, chan_in, chan_out, kernel_size, pad_mode='reflect', **kwargs):
        ...
        self.causal_padding = self.dilation * (self.kernel_size - 1) + (1 - self.stride)
        self.conv = nn.Conv1d(chan_in, chan_out, self.kernel_size, **kwargs)
...
class SoundStreamWatermark(nn.Module):
    def __init__(
            self,
            *,
            msg_len=4, # watermark message length in bits
            msg_loss_weight=1.,
            channels=32,
            strides=(2, 4, 5, 8),
        ...
        for ((chan_in, chan_out), layer_stride) in zip(chan_in_out_pairs, strides):
            encoder_blocks.append(EncoderBlock(chan_in, chan_out, layer_stride,
                                               enc_cycle_dilations, squeeze_excite, pad_mode))
        ...
        self.msg_emb = nn.Linear(msg_len, layer_channels[-1])
        self.msg_multi_scales = msg_multi_scales
        # discriminators
        ...
        self.downsamples = nn.ModuleList([nn.Identity()] + [nn.AvgPool1d(2*factor, stride=factor, padding=factor)
                                                            for factor in discr_rel_factors])
        ...
        # watermark extract
        self.ComplexSTFTMessageOut = ComplexSTFTMessageExtractor(
            msg_len=msg_len,
            stft_normalized=stft_normalized,
            logits_abs=complex_stft_discr_logits_abs  # whether to output as abs() or use view_as_real
        )
        self.MultiScaleMessageOut = MultiScaleMessageExtractor(msg_len=msg_len)
        ...
        for powers, n_fft, n_mels in zip_longest(multi_spectral_window_powers_of_two, multi_spectral_n_ffts, multi_spectral_n_mels):
            ...
            calculated_n_fft = default(max(n_fft, win_length),
                                       win_length)  # @AndreyBocharnikov said this is usually win length, but overridable
            ...
            self.mel_spec_transforms.append(melspec_transform)  # reconstruction
        ...
        self.msg_loss_weight = msg_loss_weight
    ...
    def init_and_load_from(cls, path, strict = True):
        ...
        soundstreamWatermark = cls(**config)
        soundstreamWatermark.load(path, strict = strict)
        return soundstreamWatermark
    ...
    def load(self, path, strict = True):
        ...
        if 'version' in pkg and version.parse(pkg['version']) < parsed_version:
            print(f'soundstreamWatermark model being loaded was trained on an older version of audiolm-pytorch ({pkg["version"]})')
    ...
    def non_discr_parameters(self):
        return [
            ...
            *self.msg_emb.parameters(),
            *self.MultiScaleMessageOut.parameters(),
            *self.ComplexSTFTMessageOut.parameters(),
        ]

    def non_discr_non_msg_parameters(self):
        return [
            *self.encoder.parameters(),
            *self.decoder.parameters(),
            *(self.encoder_attn.parameters() if exists(self.encoder_attn) else []),
    ...
    def encode_audio(self, x, msg_input=None, emb_msg=False):
        x = self.encoder(x)
        if emb_msg: # todo: to be determined, where to add the message
            msg_out = F.relu(self.msg_emb(msg_input))
            msg_out_repeat = msg_out.view(msg_out.size(0), msg_out.size(1), 1).repeat(1, 1, x.size(-1))
            x = x.add(msg_out_repeat)
        return x

    def decode_audio(self, x, target):
    ...
    def forward(
        ...
        return_msg_loss = False,
        return_discr_losses_separately = False,
        return_msg_losses_separately=False,
        ...
        process_input = partial(self.process_input, input_sample_hz=input_sample_hz,
                                curtail_from_left=curtail_from_left)
        x, ps = process_input(x)
        if exists(target):
            target, _ = process_input(target)
        orig_x = x.clone()

        x = self.encode_audio(x, msg_input=msg_input, emb_msg=emb_msg)
        ...
        if exists(is_denoising):
            denoise_input = torch.tensor([is_denoising, not is_denoising], dtype=x.dtype,
                                         device=self.device)  # [1, 0] for denoise, [0, 1] for not denoising
        ...
        recon_x = self.decode_audio(x, target=target)
        ...
        if return_msg_loss:
            ...
            msg_grad_penalty = None

            if self.single_channel:
                real, fake = orig_x.clone(), recon_x.detach()
                stft_msg_logits = self.ComplexSTFTMessageOut(fake)
                stft_msg_loss = nn.BCEWithLogitsLoss()(stft_msg_logits, msg_input)


            msg_logits = self.MultiScaleMessageOut(fake)
            msg_loss = nn.BCEWithLogitsLoss()(msg_logits, msg_input)

            # if apply_grad_penalty:
            #     msg_grad_penalty = gradient_penalty(real, msg_loss)

            if not return_msg_losses_separately:
                all_msg_losses = msg_loss

                if exists(stft_msg_loss):
                    all_msg_losses = all_msg_losses + stft_msg_loss

                # if exists(stft_grad_penalty):
                #     all_msg_losses = all_msg_losses + stft_grad_penalty

                return all_msg_losses

            # return a list of discriminator losses with List[Tuple[str, Tensor]]

            msg_losses_pkg = []

            msg_losses_pkg.extend([(f'msg_scale:1', msg_loss)])

            # msg_losses_pkg.extend([(f'msg_scale_grad_penalty:1', msg_grad_penalty)])

            if exists(stft_msg_loss):
                msg_losses_pkg.append(('msg_stft', stft_msg_loss))

            # if exists(stft_grad_penalty):
            #     msg_losses_pkg.append(('msg_stft_grad_penalty', stft_grad_penalty))

            return msg_losses_pkg
        ...
        if self.single_channel:
            ...
            stft_msg_loss = nn.BCEWithLogitsLoss()(stft_msg_logits, msg_input)
            # if apply_grad_penalty:
            #     stft_grad_penalty = gradient_penalty(real, stft_msg_loss)
            msg_losses.append(stft_msg_loss)

        msg_logits = self.MultiScaleMessageOut(fake)
        msg_loss = nn.BCEWithLogitsLoss()(msg_logits, msg_input)
        msg_losses.append(msg_loss)
        ...
        msg_loss = torch.stack(msg_losses).mean()
        # sum commitment loss
        ...
        total_loss = (recon_loss * self.recon_loss_weight
                      ...
                      + msg_loss * self.msg_loss_weight
                      + feature_loss * self.feature_loss_weight
                      + all_commitment_loss)
        if return_loss_breakdown:
            return total_loss, (recon_loss, multi_spectral_recon_loss, adversarial_loss, msg_loss, feature_loss, all_commitment_loss)
        return total_loss
...
from audiolm_pytorch.soundStreamWatermark import SoundStreamWatermark
...
def compare_msg_whole(msgs, msg_out, msg_len):
    correct = np.count_nonzero(
        np.sum(np.equal(msgs.detach().cpu().numpy().astype(int), msg_out.detach().cpu().numpy().astype(int)),
               axis=1) == msg_len)
    return correct
...
def compare_msg_bits(msgs, msg_out):
    correct = np.count_nonzero(
        np.equal(msgs.detach().cpu().numpy().astype(int), msg_out.detach().cpu().numpy().astype(int)) == True)
    return correct
...
class SoundStreamWatermarkTrainer(nn.Module):
    @beartype
    def __init__(
            self,
            ...
            *,
            ...
            max_grad_norm: float = 0.5,
            discr_max_grad_norm: float = None,
            msg_max_grad_norm: float = None,
            save_results_every: int = 100,
            evaluation_nums: int = 50,
            save_model_every: int = 1000,
            log_losses_every: int = 1,
            results_folder: str = './results',
            valid_frac: float = 0.05,
        ...
        self.soundstreamWatermark = soundstreamWatermark
        self.evaluation_nums = evaluation_nums
        self.use_ema = use_ema
        if self.use_ema:
            self.ema_soundstreamWatermark = EMA(soundstreamWatermark, beta = ema_beta, update_after_step = ema_update_after_step, update_every = ema_update_every)
        ...
        hyperparameters = {
            ...
            "gradient_accum_every": grad_accum_every,
            "learning_rate": lr,
            "target_sample_hz": soundstreamWatermark.target_sample_hz,
        }

        # optimizers
        self.optim = get_optimizer(soundstreamWatermark.non_discr_non_msg_parameters(), lr = lr, wd = wd)

        for discr_optimizer_key, discr in self.multiscale_discriminator_iter():
            one_multiscale_discr_optimizer = get_optimizer(discr.parameters(), lr = lr, wd = wd)
            setattr(self, discr_optimizer_key, one_multiscale_discr_optimizer)
        self.discr_optim = get_optimizer(soundstreamWatermark.stft_discriminator.parameters(), lr = lr, wd = wd,
                                         use_lion = use_lion)
        self.msg_optim = get_optimizer(soundstreamWatermark.msg_parameters(), lr = lr, wd = wd)

        # max grad norm
        self.max_grad_norm = max_grad_norm
        self.discr_max_grad_norm = discr_max_grad_norm
        self.msg_max_grad_norm = msg_max_grad_norm
        ...
        if folder is None:
            ...
            if exists(data_max_length_seconds):
                assert not exists(data_max_length)
                data_max_length = int(data_max_length_seconds * soundstreamWatermark.target_sample_hz)
            else:
                assert exists(data_max_length)
            ...
            self.ds = SoundDataset(
                ...
                max_length = data_max_length,
                target_sample_hz = soundstreamWatermark.target_sample_hz,
                seq_len_multiple_of = soundstreamWatermark.seq_len_multiple_of
            )
            # split for validation
        ...
        # prepare the multiscale discriminators with accelerator
        # prepare the msg with accelerator
        for name, _ in self.multiscale_discriminator_iter():
            optimizer = getattr(self, name)
            optimizer = self.accelerator.prepare(optimizer)
            setattr(self, name, optimizer)
        ...
        # Initialize experiment trackers if an external Accelerator is not passed in
        if not accelerator:
            self.accelerator.init_trackers("soundstreamWatermark", config=hyperparameters)

        assert self.accelerator.distributed_type != DistributedType.FSDP, 'FSDP not supported for soundstreamWatermark trainer due to complex-valued stft discriminator or message extractor'


    def set_model_as_ema_model_(self):
        """ this will force the main 'online' model to have same parameters as the exponentially moving averaged model """
        assert self.use_ema
        self.ema_soundstreamWatermark.ema_model.load_state_dict(self.soundstreamWatermark.state_dict())


    def save(self, path):
        pkg = dict(
            model = self.accelerator.get_state_dict(self.soundstreamWatermark),
            optim = self.optim.state_dict(),
            msg_optim = self.msg_optim.state_dict(),
            config = self.unwrapped_soundstreamWatermark._configs,
            discr_optim = self.discr_optim.state_dict(),
            version = __version__
        )

        if self.use_ema:
            pkg['ema_model'] = self.ema_soundstreamWatermark.state_dict()
    ...
    @property
    def unwrapped_soundstreamWatermark(self):
        return self.accelerator.unwrap_model(self.soundstreamWatermark)
    ...
    def load(self, path):
        ...
        if len(pkg.keys()) > 20:
            self.unwrapped_soundstreamWatermark.load_state_dict(pkg)

            if self.use_ema:
                self.ema_soundstreamWatermark.ema_model.load_state_dict(pkg)
            return

        # check version

        if 'version' in pkg and version.parse(pkg['version']) < version.parse(__version__):
            print(f'model was trained on older version {pkg["version"]} of soundstreamWatermark')

        # otherwise load things normally

        self.unwrapped_soundstreamWatermark.load_state_dict(pkg['model'])

        if self.use_ema:
            assert 'ema_model' in pkg
            self.ema_soundstreamWatermark.load_state_dict(pkg['ema_model'])

        self.optim.load_state_dict(pkg['optim'])
        self.discr_optim.load_state_dict(pkg['discr_optim'])
        self.msg_optim.load_state_dict(pkg['msg_optim'])
        for key, _ in self.multiscale_discriminator_iter():
            discr_optim = getattr(self, key)
            discr_optim.load_state_dict(pkg[key])
    ...
    def multiscale_discriminator_iter(self):
        for ind, discr in enumerate(self.unwrapped_soundstreamWatermark.discriminators):
            yield f'multiscale_discr_optimizer_{ind}', discr
    ...
    def train_step(self):
        ...
        self.soundstreamWatermark.train()
        ...
        for _ in range(self.grad_accum_every):
            ...
            msg = np.random.choice([0, 1], [self.batch_size, self.soundstreamWatermark.msg_len])
            msg = torch.from_numpy(msg).float()
            msg = msg.to(device)

            loss, (recon_loss, multi_spectral_recon_loss, adversarial_loss, msg_loss, feature_loss,
                   all_commitment_loss) = self.soundstreamWatermark(wave, msg_input=msg, return_loss_breakdown=True, emb_msg=True)
            ...
            if log_losses:
                accum_log(logs, dict(
                    multi_spectral_recon_loss=multi_spectral_recon_loss.item() / self.grad_accum_every,
                    msg_loss=msg_loss.item() / self.grad_accum_every,
                    adversarial_loss=adversarial_loss.item() / self.grad_accum_every,
                    feature_loss=feature_loss.item() / self.grad_accum_every,
                    all_commitment_loss=all_commitment_loss.item() / self.grad_accum_every,
                ))
        if exists(self.max_grad_norm):
            self.accelerator.clip_grad_norm_(self.soundstreamWatermark.parameters(), self.max_grad_norm)
        ...
        for _ in range(self.grad_accum_every):
            ...
            msg_losses = self.soundstreamWatermark(
                wave,
                msg_input=msg,
                apply_grad_penalty=apply_grad_penalty,
                return_msg_loss=True,
                return_msg_losses_separately=True,
                emb_msg=True
            )

            for name, msg_loss in msg_losses:
                self.accelerator.backward(msg_loss / self.grad_accum_every, retain_graph = True)
                accum_log(logs, {name: msg_loss.item() / self.grad_accum_every})
        if exists(self.msg_max_grad_norm):
            self.accelerator.clip_grad_norm_(self.soundstreamWatermark.ComplexSTFTMessageOut.parameters(), self.msg_max_grad_norm)
        self.msg_optim.step()
        ...
        for _ in range(self.grad_accum_every):
            ...
            discr_losses = self.soundstreamWatermark(
                wave,
                msg_input=msg,
                apply_grad_penalty = apply_grad_penalty,
                return_discr_loss = True,
        ...
        # build pretty printed losses
        losses_str = (f"{steps}: soundstreamWatermark total loss: {logs['loss']:.5f}, " +
                      f" reconstruct loss: {logs['recon_loss']:.5f}," +
                      f" reconstruct loss weight: {self.soundstreamWatermark.recon_loss_weight}, " +
                      f" multi_spectral reconstruct loss: {logs['multi_spectral_recon_loss']:.5f}, " +
                      f" multi_spectral reconstruct loss weight: {self.soundstreamWatermark.multi_spectral_recon_loss_weight}, " +
                      f" adversarial loss: {logs['adversarial_loss']:.5f}, " +
                      f" adversarial loss weight: {self.soundstreamWatermark.adversarial_loss_weight}, " +
                      f" message loss: {logs['msg_loss']:.5f}, " +
                      f" stft message loss: {logs['msg_stft']:.5f}, " +
                      f" message loss weight: {self.soundstreamWatermark.msg_loss_weight}, " +
                      f" feature loss: {logs['feature_loss']:.5f}, " +
                      f" feature loss weight: {self.soundstreamWatermark.feature_loss_weight}, " +
                      f" all commitment loss: {logs['all_commitment_loss']:.5f}, "
                      )
        if log_losses:
            self.accelerator.log({
                "total_loss": logs['loss'],
                ...
                "all_commitment_loss": logs['all_commitment_loss'],
                "stft_discr_loss": logs['stft'],
                "msg_loss": logs['msg_loss'],
                "msg_loss (scale 1)": logs['msg_scale:1'],
                "msg_stft": logs['msg_stft'],
            }, step=steps)
        ...
        self.accelerator.wait_for_everyone()
        if self.is_main and self.use_ema:
            self.ema_soundstreamWatermark.update()
        # sample results every so often
        self.accelerator.wait_for_everyone()

        if self.is_main and not (steps % self.save_results_every):
            stft_correct_msg_count = 0
            stft_correct_msg_count_bits = 0
            stft_tot_count = 0
            stft_tot_count_bits = 0
            multiscale_correct_msg_count = 0
            multiscale_correct_msg_count_bits = 0
            multiscale_tot_count = 0
            multiscale_tot_count_bits = 0



            models = [(self.unwrapped_soundstreamWatermark, str(steps))]
            if self.use_ema:
                models.append(
                    (self.ema_soundstreamWatermark.ema_model if self.use_ema else self.unwrapped_soundstreamWatermark, f'{steps}.ema'))
            for i in range(self.evaluation_nums):
                wave, = next(self.valid_dl_iter)
                wave = wave.to(device)
                msg = np.random.choice([0, 1], [self.batch_size, self.soundstreamWatermark.msg_len])
                msg = torch.from_numpy(msg).float()
                ...
                for model, label in models:
                    ...
                    with torch.inference_mode():
                        recons = model(wave,
                                       ...
                                       emb_msg=True)
                        stft_msg_logits = model.ComplexSTFTMessageOut(recons)
                        stft_msg_out_int = torch.round(torch.nn.Sigmoid()(stft_msg_logits))
                        stft_correct_msg_count = stft_correct_msg_count + compare_msg_whole(msg, stft_msg_out_int, self.soundstreamWatermark.msg_len)
                        stft_correct_msg_count_bits = stft_correct_msg_count_bits + compare_msg_bits(msg, stft_msg_out_int)
                        stft_tot_count = stft_tot_count + msg.shape[0]
                        stft_tot_count_bits = stft_tot_count_bits + msg.shape[0] * msg.shape[1]

                        multiscale_msg_logits = model.MultiScaleMessageOut(recons)
                        multiscale_msg_out_int = torch.round(torch.nn.Sigmoid()(multiscale_msg_logits))
                        multiscale_correct_msg_count = multiscale_correct_msg_count + compare_msg_whole(msg, multiscale_msg_out_int, self.soundstreamWatermark.msg_len)
                        multiscale_correct_msg_count_bits = multiscale_correct_msg_count_bits + compare_msg_bits(msg, multiscale_msg_out_int)
                        multiscale_tot_count = multiscale_tot_count + msg.shape[0]
                        multiscale_tot_count_bits = multiscale_tot_count_bits + msg.shape[0] * msg.shape[1]



                    for ind, recon in enumerate(recons.unbind(dim=0)):
                        filename = str(self.results_folder / f'sample_{label}.flac')
                        torchaudio.save(filename, recon.cpu().detach(), self.unwrapped_soundstreamWatermark.target_sample_hz)

            self.print(f'{steps}: saving to {str(self.results_folder)}')
            self.print(
                "up to now (to be modified to current) stft message acc: " + str(stft_correct_msg_count / stft_tot_count))
            self.print(
                "up to now (to be modified to current) stft message bit acc: " + str(stft_correct_msg_count_bits / stft_tot_count_bits))
            self.print(
                "up to now (to be modified to current) multiscale message acc: " + str(multiscale_correct_msg_count / multiscale_tot_count))
            self.print(
                "up to now (to be modified to current) multiscale message bit acc: " + str(multiscale_correct_msg_count_bits / multiscale_tot_count_bits))
        # save model every so often
        self.accelerator.wait_for_everyone()
        if self.is_main and not (steps % self.save_model_every):
            model_path = str(self.results_folder / f'soundstreamWatermark.{steps}.pt')
            self.save(model_path)
    ...
    def evaluate(self, log_fn=noop):
        # tobe completed

        self.print('to be completed evaluate function')

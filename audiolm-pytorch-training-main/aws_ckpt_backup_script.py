...
# aws cp commands in shell look like aws s3 cp /fsx/itsleonwu/audiolmWatermark-pytorch-training/sbatch_job_1.sh s3://s-laion/itsleonwu/slurm-job-1-audiolm-0.1.0/sbatch_job.sh --profile laion-stability-my-s3-bucket
...
# subprocess.run(["aws", "s3", "cp", f"/fsx/itsleonwu/audiolmWatermark-pytorch-results/{output_filename}", f"s3://{bucket_prefix}/{s3_folder}/{output_filename}", "--profile", "laion-stability-my-s3-bucket"])
